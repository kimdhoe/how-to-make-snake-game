...
const Chapter01 = ({ title, title1 }) => (
  <Chapter number={1} title={'뱀 게임 (Feeding Snake)'}>
    ...
    <Paragraph>
      ...
      우리가 가장 먼저 할 일은 연필로 종이에 그림을 그려보면서 게임이 실행 중일 떄 어떤 모습일지
    ...
    <Paragraph>
      첫번째 그림은 게임이 시작하는 순간입니다. 처음에 뱀은 머리만 있는 상태로 짙은 회색 원 한개로
      표현되었고, 붉은 원으로 표현된 먹이를 향해 움직입니다. 두번째 그림은 몸통이 길이진 뱀이 먹이를
      먹기 직전의 상태입니다. 마지막 그림은 뱀이 벽에 부딪혀 게임이 종료되는 상태입니다.
    </Paragraph>

    <Paragraph>
      자, 이제 어떤 게임인지 알았으니 프로그램을 본격적으로 구현해 볼 차례입니다.
      우리는 게임의 일부분만, 그마저도 단순화된 버전으로 먼저 구현한 뒤 프로그램에 살을 붙이면서 수정해나갈 겁니다.
      게임의 모든 요소를 한 번에 구현하려고 하면 프로그램의 구석구석이 공사 중인 상태가 되어버리겠죠.
      우리는 프로그램이 동작하는 상태를 최대한 유지하면서 차츰차츰 완성도를 높여나갈 겁니다.
    </Paragraph>
  </Chapter>
)
  ...
  >
    <div
      css={{
        ...
        paddingBottom: '100%',
        borderRadius: '50%',
        backgroundColor: food ? '#faa2c1' : '#777',
      }}
    />
  </div>
)
